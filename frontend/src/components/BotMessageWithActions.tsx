...
function parseInline(text: string): ReactNode {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[1]) {
      parts.push(<strong key={k++} className="font-semibold text-[#191919]">{match[2]}</strong>);
    } else if (match[3]) {
      parts.push(<em key={k++} className="italic text-[#37352F]">{match[4]}</em>);
    } else if (match[5]) {
      parts.push(
        <code key={k++} className="bg-[#E5F3FF] text-[#0B6E99] border border-[#BFDFF0] px-1.5 py-0.5 rounded-md text-[12px] font-mono">
          {match[6]}
        </code>
      );
    }
    ...
    <>
      {parseBlocks(content).map((block, idx) => {
        switch (block.type) {
          case 'h2':
            return (
              <h3 key={idx} className="text-[15px] font-bold text-[#191919] mt-4 mb-2 pb-1.5 border-b border-[#E9E9E7] first:mt-0">
                {parseInline(block.text)}
              </h3>
            );
          case 'h3':
            return (
              <h4 key={idx} className="text-sm font-bold text-[#191919] mt-3 mb-1 first:mt-0">
                {parseInline(block.text)}
              </h4>
            );
          case 'p':
            return (
              <p key={idx} className="text-sm text-[#37352F] leading-[1.75] mb-2 last:mb-0">
                {parseInline(block.text)}
              </p>
            );
          case 'ul':
            return (
              <ul key={idx} className="my-2 space-y-1.5">
                {block.items.map((item, i) => (
                  <li key={i} className="flex items-start gap-2.5 text-sm text-[#37352F]">
                    <span className="mt-[8px] w-1.5 h-1.5 rounded-full bg-[#0B6E99] flex-shrink-0" />
          ...
          case 'ol':
            return (
              <ol key={idx} className="my-2 space-y-2">
                {block.items.map((item, i) => (
                  <li key={i} className="flex items-start gap-3 text-sm text-[#37352F]">
                    <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#E5F3FF] text-[#0B6E99] flex items-center justify-center text-[11px] font-bold mt-0.5">
                      {i + 1}
                    </span>
                    <span className="leading-relaxed flex-1">{parseInline(item)}</span>
                  </li>
          ...
          case 'quote':
            return (
              <div key={idx} className="my-2.5 bg-[#FCF6E5] border-l-[3px] border-[#DFAB01] rounded-r-lg px-3.5 py-2.5">
                <p className="text-sm text-[#D9730D] leading-relaxed italic">{parseInline(block.text)}</p>
              </div>
            );
          case 'code':
            return (
              <div key={idx} className="my-3 rounded-md overflow-hidden border border-[#E9E9E7]">
                {block.lang && (
                  <div className="bg-[#2F2D2B] text-[#9B9A97] text-[10px] font-mono px-3 py-1.5 flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#E03E3E]" />
                    <span className="w-1.5 h-1.5 rounded-full bg-[#DFAB01]" />
                    <span className="w-1.5 h-1.5 rounded-full bg-[#0F7B6C]" />
                    <span className="ml-1 text-[#9B9A97]">{block.lang}</span>
                  </div>
                )}
                <pre className="bg-[#191919] text-[#0F7B6C] text-xs font-mono px-4 py-3 overflow-x-auto leading-relaxed whitespace-pre">
                  {block.text}
                </pre>
              </div>
            );
          case 'hr':
            return <hr key={idx} className="my-3 border-[#E9E9E7]" />;
          default:
            return null;
        }
      })}
  ...
  return (
    <div className="flex items-start gap-3 w-full group">
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Bubble */}
        <div className="bg-white rounded-md rounded-tl-sm px-4 py-3.5 border border-[#E9E9E7] min-w-0">
        ...
        <div className="flex items-center gap-0.5 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              copied
                ? 'text-[#0F7B6C] bg-[#EEF7F4]'
                : 'text-[#9B9A97] hover:text-[#787774] hover:bg-[#F7F6F3]'
            }`}
          >
            {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
            <span>{copied ? 'Copiado' : 'Copiar'}</span>
          </button>
          <span className="w-px h-4 bg-[#E9E9E7] mx-1 flex-shrink-0" />
          <button
            onClick={handleLike}
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors ${
              liked === false
                ? 'text-[#E03E3E] bg-[#FDEEEE]'
                : 'text-[#9B9A97] hover:text-[#E03E3E] hover:bg-[#FDEEEE]'
            }`}
          >
